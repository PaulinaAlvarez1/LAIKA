...
// @api
import {experinces} from '../api/data';
...
const HomeScreen = ({petExpenses, price, onChangeExpenses, onSetPrice}) => {
  ...
  const renderSaveCalculation = () => (
    <View style={styles.saveCalculationContainer}>
      ...
      <CurrencyInput
        value={petExpenses}
        onChangeValue={value => onChangeExpenses(value)}
        prefix="$"
        delimiter="."
        precision={0}
        minValue={0}
  ...
  const renderExpiriences = () => (
    <View style={styles.laikaMembersContainer}>
      <Text bold style={styles.experiencesTitleSection}>
...
const styles = StyleSheet.create({
  ...
  laikaMembersContainer: {
    alignItems: 'center',
  },
  experiencesTitleSection: {
    maxWidth: 250,
    fontSize: 25,
    marginBottom: 20,
  },
